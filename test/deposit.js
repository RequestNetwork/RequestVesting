...
contract('Deposit', function(accounts) {
	...
	it("simple deposit", async function() {
		...
		assert((await vestingERC20.tokens.call(testToken.address, spender2)).equals(0), "spender2SupplyOnContract is wrong");
	});
	...
	it("deposit without approval", async function() {		
		...
		assert((await vestingERC20.tokens.call(testToken.address, spender1)).equals(0), "spender1SupplyOnContract is wrong");
	...
	it("double deposit one approval", async function() {		
		...
		assert((await vestingERC20.tokens.call(testToken.address, spender1)).equals(spender1Supply.div(2)), "spender1SupplyOnContract is wrong");
	...
	it("two deposit from different tokens with two users", async function() {		
		...
		assert(r.logs[0].args.amount.equals(spender1Supply), "amount is wrong");
		assert(r.logs[0].args.balance.equals(spender1Supply), "balance is wrong");
		assert((await vestingERC20.tokens.call(testToken2.address, spender1)).equals(spender1Supply), "spender1SupplyOnContract is wrong");
		...
		assert(r.logs[0].args.amount.equals(spender2Supply), "amount is wrong");
		assert(r.logs[0].args.balance.equals(spender2Supply), "balance is wrong");
		assert((await vestingERC20.tokens.call(testToken2.address, spender2)).equals(spender2Supply), "spender2SupplyOnContract is wrong");
	});
