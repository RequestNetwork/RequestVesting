...
contract('Grant', function(accounts) {
	...
	it("Grant Vesting for himself", async function() {
		...
		// create the grant
		var r = await vestingERC20.createVesting(testToken.address, spender1, grantSpender1toVester1, 
											startTimeSolidity, grantPeriodS1V1, cliffPeriodS1V1,
											 {from: spender1});
		...
		assert(r.logs[0].args.vestedAmount.equals(grantSpender1toVester1), "vestedAmount is wrong");
		assert.equal(r.logs[0].args.granter, spender1, "from is wrong");
		assert.equal(r.logs[0].args.vester, spender1, "to is wrong");
		assert(r.logs[0].args.startTime.equals(startTimeSolidity), "startTime is wrong");
		assert(r.logs[0].args.cliffTime.equals(startTimeSolidity+cliffPeriodS1V1), "cliffTime is wrong");
		assert(r.logs[0].args.endTime.equals(startTimeSolidity+grantPeriodS1V1), "endTime is wrong");
		...
		var grantsS1toV1 = await vestingERC20.grantPerTokenGranterVester.call(testToken.address, spender1, spender1);
		assert(grantsS1toV1[0].equals(grantSpender1toVester1), "vestedAmount is wrong");
		assert.equal(grantsS1toV1[1], startTimeSolidity, "startTime is wrong");
		assert.equal(grantsS1toV1[2], startTimeSolidity+cliffPeriodS1V1, "cliffTime is wrong");
		assert.equal(grantsS1toV1[3], startTimeSolidity+grantPeriodS1V1, "endtime is wrong");
	...
	it("Grant Vesting with wrong arguments", async function() {
		...
		// createVesting with _token == 0
		await expectThrow(vestingERC20.createVesting(0,
														vester1, 
														grantAmount,    
								            startTimeSolidity,
								            grantPeriod,
		...
		// createVesting with _to == 0
		await expectThrow(vestingERC20.createVesting(testToken.address,
														0, 
														grantAmount,    
								            startTimeSolidity,
								            grantPeriod,
		...
		// createVesting with vestedAmount 0
		await expectThrow(vestingERC20.createVesting(testToken.address,
														vester1, 
														0,    
								            startTimeSolidity,
		...
		// createVesting with cliffPeriod > grantPeriod
		await expectThrow(vestingERC20.createVesting(testToken.address,
														vester1, 
													grantAmount,
													startTimeSolidity,
		...
		var r = await vestingERC20.createVesting(testToken.address,
														vester1, 
														grantAmount,    
								            startTimeSolidity,
								            grantPeriod,
		...
		// createVesting to someone with already a grant
		await expectThrow(vestingERC20.createVesting(testToken.address,
														vester1, 
														grantAmount,    
								            startTimeSolidity,
		...
		// createVesting with vestedAmount > token available
		await expectThrow(vestingERC20.createVesting(testToken.address,
												vester3, 
												grantAmount,    
						            startTimeSolidity,
	...
	it("grant vesting with same token and same from to different vesters", async function() {
		...
		var r = await vestingERC20.createVesting(testToken.address, vester2, grantSpender1toVester2, 
											startTimeSolidity, grantPeriodS1V1, cliffPeriodS1V1,
											 {from: spender1});
		...
		assert((await vestingERC20.getContractBalance.call(testToken.address, spender1)).equals(spender1Supply.minus(grantSpender1toVester1).minus(grantSpender1toVester2)), "spender1SupplyOnContract is wrong");

		var grantsS1toV1 = await vestingERC20.grantPerTokenGranterVester.call(testToken.address, spender1, vester2);
	...
	it("grant vesting to the same vester but different token", async function() {
		...
		var r = await vestingERC20.createVesting(testToken2.address, vester1, grantSpender1toVester1Token2, 
											startTimeSolidity, grantPeriodS1V1, cliffPeriodS1V1,
											 {from: spender1});
		...
		assert((await vestingERC20.getContractBalance.call(testToken2.address, spender1)).equals(spender1Supply.minus(grantSpender1toVester1Token2)), "spender1SupplyOnContract is wrong");

		var grantsS1toV1 = await vestingERC20.grantPerTokenGranterVester.call(testToken2.address, spender1, vester1);
	...
	it("grant vesting to someone with a vesting same token but from other spender", async function() {
		...
		// create the grant token 2
		var r = await vestingERC20.createVesting(testToken.address, vester1, grantSpender1toVester1fromSpender2, 
											startTimeSolidity, grantPeriodS1V1, cliffPeriodS1V1,
											 {from: spender2});
		...
		assert.equal(r.logs[0].args.granter, spender2, "from is wrong");
		assert.equal(r.logs[0].args.vester, vester1, "to is wrong");
		assert(r.logs[0].args.startTime.equals(startTimeSolidity), "startTime is wrong");
		assert(r.logs[0].args.cliffTime.equals(startTimeSolidity+cliffPeriodS1V1), "cliffTime is wrong");
		assert(r.logs[0].args.endTime.equals(startTimeSolidity+grantPeriodS1V1), "endTime is wrong");

		assert((await vestingERC20.getContractBalance.call(testToken.address, spender2)).equals(spender2Supply.minus(grantSpender1toVester1fromSpender2)), "spender1SupplyOnContract is wrong");

		var grantsS1toV1 = await vestingERC20.grantPerTokenGranterVester.call(testToken.address, spender2, vester1);
	...
	it("grant to withdraw balance deposit", async function() {
		...
		assert((await vestingERC20.getVestingBalance.call(testToken.address, spender1, spender1)).equals(grantSpender1toVester1), "spender1SupplyOnContract is wrong");
